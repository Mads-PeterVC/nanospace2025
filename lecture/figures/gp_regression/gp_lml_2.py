...
xm = np.array([0.1, 0.2, 0.33, 0.4, 0.5, 0.66, 0.99, 0.68, 0.8, 0.85, 0.9])
ym = np.array([-1, 0.2, 0.2, 0.5, -0.2, 1.0, -1, 2, 1.5, 0.8, 0])


sz = 4
fig, ax = plt.subplots(1, 1, figsize=(1.5 * sz, 1 * sz), constrained_layout=True)

noise_values = np.linspace(1e-3, 2, 500)
...
plt.savefig("figures/2_gp_lml.png", dpi=300, bbox_inches="tight")
...
for i, (ls, n) in enumerate(zip(length_scale, noise)):
    ...
    ax.legend()
    plt.savefig(f'figures/2_gp_lml_{i}.png', dpi=300, bbox_inches='tight')
