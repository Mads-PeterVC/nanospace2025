...
def sample_posterior(noise=1e-3, length_scale=0.1):
...
for i, l in enumerate([0.01, 0.1, 0.25, 0.5]):
    ...
    mean_post, samples_post, cov_post = sample_posterior(noise=1e-3, length_scale=l)
    ...
    ax[i].set_title(f'$\lambda={l:0.3f}$')
    ax[i].set_xlabel('$X$')
    ax[i].set_xlim((0, 1))
    ax[i].set_ylim((-3, 3))
