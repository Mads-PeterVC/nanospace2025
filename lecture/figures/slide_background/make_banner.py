...
# 16x9 aspect ratio
fig, ax = plt.subplots(figsize=(2*16, 2*1), layout='constrained')
ax.plot(X_query, samples.T, color="gray", linewidth=0.25, alpha=1.0)
ax.plot(X_query, prediction.mean, color="black", linewidth=2)
ax.fill_between(X_query.flatten(),
                prediction.mean - 2*np.sqrt(prediction.variance),
                prediction.mean + 2*np.sqrt(prediction.variance),
                color="gray", alpha=0.1, label='Uncertainty', edgecolor='mediumpurple')
...
# plt.savefig('slide_banner.png', dpi=600, bbox_inches='tight', transparent=True)
plt.savefig('slide_banner.pdf', dpi=600, bbox_inches='tight', transparent=True)
